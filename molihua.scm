  ...
  (do ((v 0 (+ v 1)))
    ...
    (let* ((indices (chamfer v))
      ...
      (let* ((corner (vector-ref vertices v))
             (points (map (lambda (i)
                            (if (pair? i)
                                (cons (vector-ref offset-vertices (car i))
                                      (vector-ref offset-vertices (cdr i)))
                                (vector-ref offset-vertices i)))
                          indices))
             (face-indices (vector-ref vertex-faces v))
             (polys (map (lambda (i)
        ...
        (let loop ((i 0))
          (when (< i n)
            (let* ((j (modulo (+ i 1) n))
                   ...
                   (q1 (list-ref points i))
                   (p1 (if (pair? (car q1)) (car q1) q1))
                   (q2 (list-ref points j))
                   (p2 (if (pair? (car q2)) (cdr q2) q2)))
              (set! misc-lines
